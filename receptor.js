...
Receptor.prototype.listen = function( callback )
{
	var self = this,
		...
		Configure = function()
		{
			...
			// set process title
			if( !conf.ServerName ){
				conf.ServerName = conf.Listen;
				// process.title = 'node-kahana@' + conf.ServerName + ':' + conf.Port;
			}
			else {
				// process.title = conf.ServerName + ':' + conf.Port;
			}
			// set Listen
			...
			// set KeepAlive
			conf.KeepAlive = ( String( conf.KeepAlive ).toLowerCase() == 'on' ) ? true : false;
			// set DirectorySlash
			conf.DirectorySlash = ( String( conf.DirectorySlash ).toLowerCase() == 'on' ) ? true : false;
			// set Cached
			conf.Cached = ( String( conf.Cached ).toLowerCase() == 'on' ) ? true : false;
			// set PidFile
			if( conf.PidFile )
			{
				var pidfd;
				...
				try {
					conf.PidFile = pkg.fs.realpathSync( conf.PidFile );
				}catch(e){};
